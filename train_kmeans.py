...
from scene.kmeans_quantize import Quantize_kMeans, Quantize_AdditiveColor
try:
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    n_cls_color_a = args.kmeans_ncls_color_a
    n_cls_color_b = args.kmeans_ncls_color_b
    ...
    if 'add_sh_dc' in quantized_params:
        kmeans_addshdc_q = Quantize_AdditiveColor(num_clusters_a=n_cls_color_a, num_clusters_b=n_cls_color_b, num_iters=n_it)

    force_kmeans_assign = False
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if iteration > kmeans_st_iter:
            if force_kmeans_assign:
                assign = True
                force_kmeans_assign = False
            elif iteration % freq_cls_assn == 1:
                assign = True
            else:
                assign = False
            if 'pos' in quantized_params:
            ...
            if 'add_sh_dc' in quantized_params:
                kmeans_addshdc_q.forward_dcfrest(gaussians, assign=assign)
        ...
        if args.opacity_reg:
            if iteration > args.max_prune_iter or iteration < args.opacity_reg_start_iter:
                lambda_reg = 0.
            else:
                lambda_reg = args.lambda_reg
            L_reg_op = gaussians.get_opacity.mean()
        ...
        with torch.no_grad():
            ...
            if (iteration in saving_iterations):
                ...
                save_quantized_keys = set(quantized_params)
                if 'add_sh_dc' in save_quantized_keys:
                    save_quantized_keys.update(['dc', 'sh'])
                if 'lowrank_sh' in save_quantized_keys:
                    save_quantized_keys.update(['sh'])
                save_attributes = [val for (key, val) in all_attributes.items() if key not in save_quantized_keys]
                if iteration > kmeans_st_iter:
                    ...
                    for param in quantized_params:
                        if param == 'add_sh_dc':
                            kmeans_list.extend([kmeans_addshdc_q.q_a, kmeans_addshdc_q.q_b])
                            kmeans_save_params.extend(['add_sh_dc_a', 'add_sh_dc_b'])
                            kmeans_all_params.extend(['add_sh_dc_a', 'add_sh_dc_b'])
                        elif param == 'lowrank_sh':
                            extra_centers.update(build_lowrank_sh_state(gaussians, args.lowrank_sh_rank))
                            kmeans_all_params.append('lowrank_sh')
                        else:
                            kmeans_list.append(kmeans_dict[param])
                            kmeans_save_params.append(param)
                            kmeans_all_params.append(param)
                    out_dir = join(scene.model_path, 'point_cloud/iteration_%d' % iteration)
                    save_kmeans(kmeans_list, kmeans_all_params, out_dir, extra_centers=extra_centers, index_params=kmeans_save_params)
                else:
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    force_kmeans_assign = True

                if iteration % opt.opacity_reset_interval == 0 or (dataset.white_background and iteration == opt.densify_from_iter):
                    gaussians.reset_opacity()

            # Optional opacity pruning after opacity regularization.
            do_opacity_prune = args.opacity_reg and iteration > args.opacity_prune_start_iter
            if args.disable_lowrank_opacity_prune and 'lowrank_sh' in args.quant_params:
                do_opacity_prune = False
            if do_opacity_prune:
                if iteration <= args.max_prune_iter and iteration % args.opacity_prune_interval == 0:
                    print('Num Gaussians: ', gaussians._xyz.shape[0])
                    size_threshold = None
                    gaussians.prune(
                        args.opacity_prune_threshold,
                        scene.cameras_extent,
                        size_threshold,
                        scale_aware=args.scale_aware_pruning,
                        scale_quantile=args.scale_prune_quantile,
                    )
                    force_kmeans_assign = True
...
def build_lowrank_sh_state(gaussians, rank):
    """Store SH as continuous low-rank coefficients plus shared basis."""
    with torch.no_grad():
        feat = gaussians._features_rest.detach()
        flat = feat.reshape(feat.shape[0], -1).float()
        rank = min(int(rank), flat.shape[0], flat.shape[1])
        ...
        basis = eigvecs[:, -rank:].T.contiguous()
        coeffs = centered @ basis.T

        recon = coeffs @ basis + mean
        rmse = torch.sqrt(torch.mean((flat - recon) ** 2)).item()
        print(f"Low-rank SH rank: {rank}, RMSE: {rmse:.8f}")

        return {
            'lowrank_sh_basis': basis.cpu(),
            'lowrank_sh_coeffs': coeffs.cpu(),
            'lowrank_sh_mean': mean.squeeze(0).cpu(),
        }


def save_kmeans(kmeans_list, quantized_params, out_dir, extra_centers=None, index_params=None):
    """Save KMeans indices/codebooks plus optional non-index compressed tensors."""
    if index_params is None:
        index_params = quantized_params
    if extra_centers is None:
        extra_centers = {}
    ...
    args_dict['index_params'] = index_params
    args_dict['n_bits'] = n_bits
    args_dict['total_len'] = len(bitarray_all)
    np.save(join(out_dir, 'kmeans_args.npy'), args_dict)

    centers_dict = {param: kmeans.centers for (kmeans, param) in zip(kmeans_list, index_params)}
    centers_dict.update(extra_centers)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--kmeans_ncls_color_a', type=int, default=4096,
                        help='number of clusters for additive color codebook A')
    parser.add_argument('--kmeans_ncls_color_b', type=int, default=4096,
                        help='number of clusters for additive color codebook B')
    parser.add_argument('--lowrank_sh_rank', type=int, default=16,
                        help='rank for low-rank SH basis compression')
    ...
    parser.add_argument('--opacity_reg_start_iter', type=int, default=15000,
                        help='Iteration from which opacity regularization starts')
    parser.add_argument('--opacity_prune_start_iter', type=int, default=15000,
                        help='Iteration from which opacity-based pruning starts')
    parser.add_argument('--opacity_prune_interval', type=int, default=1000,
                        help='Interval for opacity-based pruning')
    parser.add_argument('--opacity_prune_threshold', type=float, default=0.005,
                        help='Opacity threshold used by opacity-based pruning')
    parser.add_argument('--scale_aware_pruning', action='store_true', default=False,
                        help='Use scale-aware opacity pruning during opacity regularization')
    parser.add_argument('--scale_prune_quantile', type=float, default=0.5,
                        help='Preserve low-opacity Gaussians whose max scale is above this quantile')
    parser.add_argument('--disable_lowrank_opacity_prune', action='store_true', default=False,
                        help='Disable opacity pruning when lowrank_sh is enabled')
    ...
    if 'add_sh_dc' in args.quant_params and ('sh' in args.quant_params or 'dc' in args.quant_params):
        raise ValueError('add_sh_dc cannot be used together with sh or dc in --quant_params')
    if 'lowrank_sh' in args.quant_params and ('sh' in args.quant_params or 'add_sh_dc' in args.quant_params):
        raise ValueError('lowrank_sh cannot be used together with sh or add_sh_dc in --quant_params')
