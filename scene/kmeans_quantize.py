...
class Quantize_AdditiveColor():
    """Two-codebook additive quantization for concatenated [dc, sh] color features."""
    def __init__(self, num_clusters_a=4096, num_clusters_b=4096, num_iters=1):
        self.q_a = Quantize_kMeans(num_clusters=num_clusters_a, num_iters=num_iters)
        self.q_b = Quantize_kMeans(num_clusters=num_clusters_b, num_iters=num_iters)

    def forward_dcfrest(self, gaussian, assign=False):
        feat = torch.cat([gaussian._features_dc, gaussian._features_rest], 1)
        vec_dim = feat.shape[1] * feat.shape[2]
        if self.q_a.vec_dim == 0:
            self.q_a.vec_dim = vec_dim
        if self.q_b.vec_dim == 0:
            self.q_b.vec_dim = vec_dim

        flat = feat.reshape(-1, vec_dim)

        if assign:
            self.q_a.cluster_assign(feat)
        else:
            self.q_a.update_centers(feat)
        sampled_a = torch.gather(
            self.q_a.centers, 0, self.q_a.nn_index.unsqueeze(-1).repeat(1, vec_dim)
        )

        residual = flat.detach() - sampled_a.detach()
        if assign:
            self.q_b.cluster_assign(residual)
        else:
            self.q_b.update_centers(residual)
        sampled_b = torch.gather(
            self.q_b.centers, 0, self.q_b.nn_index.unsqueeze(-1).repeat(1, vec_dim)
        )

        sampled = sampled_a + sampled_b
        quant_flat = flat - flat.detach() + sampled
        quant_feat = quant_flat.reshape(-1, feat.shape[1], feat.shape[2])

        dc_deg = gaussian._features_dc.shape[1]
        gaussian._features_dc_q = quant_feat[:, :dc_deg, :]
        gaussian._features_rest_q = quant_feat[:, dc_deg:, :]
