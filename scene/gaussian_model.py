# Code from original 3D Gaussian Splat repo
#
...
class GaussianModel:
    ...
    def load_ply(self, path, load_quant=False):
        ...
        if load_quant:
            ...
            index_params = args_dict.get('index_params', quant_params)
            ...
            if len(index_params) > 0:
                ...
                indices = np.reshape(indices.cpu().numpy(), (len(index_params), -1))
                for i, key in enumerate(index_params):
                    indices_dict[key] = indices[i]

        has_add_sh_dc = ('add_sh_dc_a' in quant_params) and ('add_sh_dc_b' in quant_params)
        has_lowrank_sh = 'lowrank_sh' in quant_params
        ...
        if has_add_sh_dc:
            color_features = (codebook['add_sh_dc_a'][indices_dict['add_sh_dc_a']] +
                              codebook['add_sh_dc_b'][indices_dict['add_sh_dc_b']]).cpu().numpy()
            features_dc = np.expand_dims(color_features[:, :3], -1)
        elif 'dc' in quant_params:
        ...
        if has_lowrank_sh:
            basis = codebook['lowrank_sh_basis'].float()
            coeffs = codebook['lowrank_sh_coeffs'].float()
            mean = codebook['lowrank_sh_mean'].float()
            features_extra = (coeffs @ basis + mean).cpu().numpy()
            ...
            features_extra = color_features[:, 3:]
            features_extra = features_extra.reshape((features_extra.shape[0], (self.max_sh_degree + 1) ** 2 - 1, 3))
            features_extra = features_extra.transpose((0, 2, 1))
        elif 'sh' in quant_params:
    ...
    def prune(self, min_opacity, extent, max_screen_size, scale_aware=False, scale_quantile=0.5):
        low_opacity_mask = (self.get_opacity < min_opacity).squeeze()
        if scale_aware and low_opacity_mask.any():
            scale_extent = self.get_scaling.detach().max(dim=1).values
            scale_threshold = torch.quantile(scale_extent, scale_quantile)
            small_scale_mask = scale_extent < scale_threshold
            prune_mask = torch.logical_and(low_opacity_mask, small_scale_mask)
        else:
